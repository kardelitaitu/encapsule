...
// The credential redactor is not here any more: `scrub_token`,
// `is_proxy_flag_token` and `sanitize_parse_error` moved to <utils.hpp>, as
// `encapsule::utils`, beside the `parse_proxy_url` whose split they reuse.
// They were `static` in this file, which is why no test could assert what
// they do -- and what they do is keep a socks5 password out of stderr, the
// console scrollback and any CI transcript.  The cases that pin that are in
// `tests/utils/utils_test.cpp`; the argparse-v3 gap they deliberately do not
// yet close is documented there.
using encapsule::utils::sanitize_parse_error;
